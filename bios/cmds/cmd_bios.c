 ...
 *
 */
static void hello_handler(int nb_params, char **params)
{
	char *c;
	unsigned int value;
...
define_command(hello, hello_handler, "Print hello", SYSTEM_CMDS);
...
static void cmatrix_handler(int nb_params, char **params) {
  ...
  while (1) {
      ...
      if (t[i] > 280)t[i] = 0;
		...
		inter_time = timer0_uptime_cycles_read();

    screen_refresh();
  }
}

define_command(cmatrix, cmatrix_handler, "Cmatrix", SYSTEM_CMDS);
 ...
 *
 */
#ifdef CSR_coremark_BASE
static void coremark_handler(int nb_params, char **params)
{
	char *c;
	unsigned int value;

	if (nb_params < 1) {
		printf("coremark <value>");
		return;
	}
...
define_command(coremark, coremark_handler, "CoreMark is the Benchmark", SYSTEM_CMDS);
