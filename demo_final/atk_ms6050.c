...
#define SEL_ATK_MS6050          1

uint8_t atk_ms6050_write(uint8_t addr,uint8_t reg, uint8_t len, uint8_t *dat)
{
    uint8_t i;
    
    iic_start(SEL_ATK_MS6050);
    iic_send_byte((addr << 1) | 0, SEL_ATK_MS6050);
    if (iic_wait_ack(SEL_ATK_MS6050) == 1)
    {
        iic_stop(SEL_ATK_MS6050);
        return ATK_MS6050_EACK;
    }
    iic_send_byte(reg, SEL_ATK_MS6050);
    if (iic_wait_ack(SEL_ATK_MS6050) == 1)
    {
        iic_stop(SEL_ATK_MS6050);
        return ATK_MS6050_EACK;
    ...
    for (i=0; i<len; i++)
    {
        iic_send_byte(dat[i], SEL_ATK_MS6050);
        if (iic_wait_ack(SEL_ATK_MS6050) == 1)
        {
            iic_stop(SEL_ATK_MS6050);
            return ATK_MS6050_EACK;
...
uint8_t atk_ms6050_read(uint8_t addr, uint8_t reg, uint8_t len, uint8_t *dat)
{
    ...
    iic_start(SEL_ATK_MS6050);
    iic_send_byte((addr << 1) | 1, SEL_ATK_MS6050);
    if (iic_wait_ack(SEL_ATK_MS6050) == 1)
    {
        iic_stop(SEL_ATK_MS6050);
        return ATK_MS6050_EACK;
    ...
    while (len)
    {
        *dat = iic_read_byte((len > 1) ? 1 : 0, SEL_ATK_MS6050);
        len--;
        dat++;
    }
    iic_stop(SEL_ATK_MS6050);
