...
/* fabs is for doubles. fabsf is for ints. */
#define fabs        fabsf
...
int mpu_get_temperature(long *data, unsigned long *timestamp)
{
    ...
    data[0] = (long)((35 + ((raw - (int)st.hw->temp_offset) / st.hw->temp_sens)) * 65536L);
    return 0;
}
 ...
 *  @return     0 if successful.
 */
int mpu_get_gyro_sens(int *sens)
{
    switch (st.chip_cfg.gyro_fsr) {
    case INV_FSR_250DPS:
        sens[0] = 131;
...
#ifdef MPU6050
static int get_accel_prod_shift(int *st_shift)
{
...
#ifdef EMPL_NO_64BIT
    gyro[0] = (long)(((int)gyro[0]*65536.f) / test.gyro_sens / packet_count);
    gyro[1] = (long)(((int)gyro[1]*65536.f) / test.gyro_sens / packet_count);
    gyro[2] = (long)(((int)gyro[2]*65536.f) / test.gyro_sens / packet_count);
    if (has_accel) {
        accel[0] = (long)(((int)accel[0]*65536.f) / test.accel_sens /
            packet_count);
        accel[1] = (long)(((int)accel[1]*65536.f) / test.accel_sens /
            packet_count);
        accel[2] = (long)(((int)accel[2]*65536.f) / test.accel_sens /
            packet_count);
 ...
 *              1: 函数执行失败
 */
uint8_t atk_ms6050_dmp_get_data(int *pitch, int *roll, int *yaw)
{
    int q0 = 0;
    int q1 = 0;
    int q2 = 0;
