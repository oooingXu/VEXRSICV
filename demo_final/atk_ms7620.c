...
#define ATK_MS7620_REG_ENABLE               0x72
...
{
    iic_start(SEL_ATK_MS7620);
    iic_send_byte((addr << 1) | 0, SEL_ATK_MS7620);
    if (iic_wait_ack(SEL_ATK_MS7620) == 1)
    {
        iic_stop(SEL_ATK_MS7620);
        return ATK_MS7620_EACK;
    }
    iic_send_byte(reg, SEL_ATK_MS7620);
    if (iic_wait_ack(SEL_ATK_MS7620) == 1)
    {
        iic_stop(SEL_ATK_MS7620);
        return ATK_MS7620_EACK;
    ...
    iic_start(SEL_ATK_MS7620);
    iic_send_byte((addr << 1) | 1, SEL_ATK_MS7620);
    if (iic_wait_ack(SEL_ATK_MS7620) == 1)
    {
        iic_stop(SEL_ATK_MS7620);
        return ATK_MS7620_EACK;
    ...
    while (len)
    {
        *dat = iic_read_byte((len > 1) ? 1 : 0, SEL_ATK_MS7620);
        len--;
        dat++;
    }
    iic_stop(SEL_ATK_MS7620);
 ...
 *              ATK_MS7620_EACK: IIC通讯ACK错误，函数执行失败
 */
uint8_t atk_ms7620_write(uint8_t addr,uint8_t reg, uint8_t len, uint8_t *dat)
{
    uint8_t i;
    ...
    for (i=0; i<len; i++)
    {
        iic_send_byte(dat[i], SEL_ATK_MS7620);
        if (iic_wait_ack(SEL_ATK_MS7620) == 1)
        {
            iic_stop(SEL_ATK_MS7620);
            return ATK_MS7620_EACK;
 ...
 *              ATK_MS7620_EACK: IIC通讯ACK错误，函数执行失败
 */
uint8_t atk_ms7620_write_byte(uint8_t addr, uint8_t reg, uint8_t dat)
{
 ...
 * @retval      无
 */
void atk_ms7620_switch_reg_bank(atk_ms7620_bank_t bank)
{
    switch (bank)
    {
...
static uint8_t atk_ms7620_get_wakeup_status(void)
{
  ...
  /*----------wakeup----------*/
    iic_start(SEL_ATK_MS7620);
    iic_send_byte(ATK_MS7620_IIC_ADDR << 1, SEL_ATK_MS7620);
    iic_stop(SEL_ATK_MS7620);    
    delay_us(5000);
    iic_start(SEL_ATK_MS7620);
    iic_send_byte(ATK_MS7620_IIC_ADDR << 1, SEL_ATK_MS7620);
...
uint8_t atk_ms7620_init(void)
{
  ...
  /*-----------------guaqi----------------
    atk_ms7620_switch_reg_bank(ATK_MS7620_BANK1);
    atk_ms7620_write_byte(ATK_MS7620_IIC_ADDR,0x72,0x00);
    atk_ms7620_switch_reg_bank(ATK_MS7620_BANK0);
    atk_ms7620_write_byte(ATK_MS7620_IIC_ADDR,0x03,0x01);
  ------------------eggs----------------*/
    ...
    atk_ms7620_switch_reg_bank(ATK_MS7620_BANK0);/*eggs*/
