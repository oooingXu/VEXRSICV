...
static inline void uart_set_color(int c) {
  printf("\033[%dm", c);
}

static inline void uart_print_char(char c, int y, int x) {
  printf("\033[%d;%dH%c", y + 1, x + 1, c);
}

static inline void uart_screen_clear() {
  printf("\033[H\033[J");
}

static inline void uart_screen_refresh() {
}

static inline int uart_screen_tile_height() {
  return 24;
}

static inline int uart_screen_tile_width() {
  return 80;
}
