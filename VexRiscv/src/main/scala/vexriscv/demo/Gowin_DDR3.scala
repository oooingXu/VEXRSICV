...
case class Gowin_DDR3(sysclk : ClockDomain, memclk : ClockDomain) extends BlackBox {
  ...
  val io = new Bundle {
    ...
    */
    ...
    val IO_ddr_dq = inout(Analog(Bits(16 bits)))  /* wxz */
    val IO_ddr_dqs = inout(Analog(Bits(2 bits)))   /* wxz */
    val IO_ddr_dqs_n = inout(Analog(Bits(2 bits))) /* wxz */
  ...
  def connectDDR3Interface(iface: DDR3_Interface): Unit = {
   ...
   iface.pll_lock <> io.pll_lock /*wxz*/
  }
